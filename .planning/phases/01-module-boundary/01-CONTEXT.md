...
- Internal files consolidate into 3 headers: `tui_ncurses.h` (public component types + module), `tui_internal.h` (implementation details), `tui_draw.h` (drawing API unchanged)
...
- Standard CELS patterns: both `cel_module_requires(NCurses)` and `cel_module_load(NCurses)` work
...
- Window system uses observer pattern — watches for NCurses_WindowConfig being added to an entity, initializes terminal reactively
